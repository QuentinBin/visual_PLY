...
from dexnet.grasping import PointGrasp, GraspableObject3D, GraspQualityConfig, CylinderPoint3D
...
class PointGraspMetrics3D:
    ...
    def grasp_quality(grasp, obj, params, vis=False):
        ...
        check_approach_ = params.check_approach
        if not hasattr(PointGraspMetrics3D, method):
            raise ValueError('Illegal point grasp metric %s specified' % (method))


        if method == 'my_force_closure':
            # close_fingers( center, axis, grasp_depth, radius, obj, vis=False, check_approach=True, approach_dist=1.0):
            print(type(obj))
            contacts_found, contacts = CylinderPoint3D.close_fingers(grasp.center_, grasp.axis_, grasp.grasp_depth_, grasp.max_grasp_width_, obj, check_approach=check_approach_)
            if not contacts_found:
                print('Contacts not found')
                return 0
            else:
                return PointGraspMetrics3D.my_force_closure(contacts, grasp.axis_ , friction_coef)

        # if method == 'force_closure':
        #     # get point grasp contacts
        #     contacts_start = time.time()
        #     contacts_found, contacts = grasp.close_fingers(obj, check_approach=check_approach_, vis=vis)
        #     if not contacts_found:
    ...
    @staticmethod
    def my_force_closure(contacts, grasp_axis, friction_coef, use_abs_value=True):
        """" Checks force closure using the antipodality trick.
        ...
        for k, contact in enumerate(contacts):
            ...
            # print(contact.normal_)
            if contact is None or contact.point_ is None or contact.normal_ is None:
                continue
            
            point = contact.point_
            normal = contact.normal_

            normal = normal / np.linalg.norm(contact.normal)
            grasp_axis = grasp_axis / np.linalg.norm(grasp_axis)

            axis_normal = grasp_axis.dot(normal) #正压力分力
            total_axis_normal += axis_normal
            theta = np.arccos(axis_normal)
            # print("sintheta:",np.sin(theta))
            # print("norm:",np.linalg.norm(normal))
            # print("axis_normal:",axis_normal)
            total_axis_friction += abs(np.linalg.norm(normal)* friction_coef * np.sin(theta))

        if  total_axis_friction > abs(total_axis_normal):
            print("grasp is force closed")
            print("total_axis_friction:",total_axis_friction)
            print("abs(total_axis_normal):", abs(total_axis_normal))
            return  abs(total_axis_normal)/ total_axis_friction * friction_coef
        else:
            print("grasp is not force closed")
            print("total_axis_friction:",total_axis_friction)
            print("abs(total_axis_normal):", abs(total_axis_normal))
            return 0

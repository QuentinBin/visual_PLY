...
class Heatmap_Generate(object):
    ...
    def generate_quality(self):
        ...
        max_quality = 0
        for i in range(0,self.picture_size_[0],4):
            for j in range(0,self.picture_size_[1],3):
                ...
                if index != -1:
                    ...
                    if mu > 0:
                        quality =  (1/mu) # grasp_depth/self.gripper_config_[0] *
    ...
    def generate_best_grasp(self):
        T_camera_in_obj = self.T_table_in_obj_.dot(self.T_camera_in_table_)
        grasp_axis_in_obj = T_camera_in_obj[:3, 2] #- self.T_camera_in_table_[:3, 2] # verical grasp
        (row_index, col_index) = np.where(self.index_img_!=-1)
        initial_index = random.randint(0, row_index.shape[0]-1) # chose initial coodinates
        row_0 = row_index[initial_index]
        col_0 = col_index[initial_index]
        grasp_center_in_obj = self.surfpoints_in_obj_[self.index_img_[row_0,col_0]]
        wrapped_surfs, grasp_depth = self.generate_grasp(grasp_axis_in_obj, grasp_center_in_obj)
        mu = self.force_closure(wrapped_surfs, grasp_axis_in_obj)
        if mu > 0:
            quality_0 = (1/mu) # grasp_depth/self.gripper_config_[0] *
        else:
            quality_0 = 0

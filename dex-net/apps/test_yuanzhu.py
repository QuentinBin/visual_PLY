...
envpath = '/home/peng/miniconda3/envs/grasp/lib/python3.7/site-packages/cv2/qt/plugins'
...
def display_gripper_on_object(obj_, grasp_):
    ...
    t_obj_gripper = ParallelJawPtGrasp3D.gripper_pose(gripper)
    # class ParallelJawPtGrasp3D(PointGrasp): 
    stable_pose = t_obj_gripper
    grasp_ = ParallelJawPtGrasp3D.perpendicular_table(stable_pose)
...
def display_grasps(grasp, graspable, color):
    #  grasps_with_score   shape(1000,12);  (0,1,2) 当前姿态抓取中心点坐标；(3,4,5) 
    center_point = grasp[0:3]
    print('center_point',center_point)
    column_axis = grasp[3:6]  # 轴向
    grasp_depth = grasp[6]
    radius = grasp[7]

    Bottom_center = center_point + column_axis * grasp_depth

    print('column_axis,grasp_depth,radius,Bottom_center',column_axis,grasp_depth,radius,Bottom_center)
    
    mlab.points3d(center_point[0],center_point[1],center_point[2],scale_factor=0.005,color = (0,0,0))
    mlab.points3d(Bottom_center[0],Bottom_center[1],Bottom_center[2],scale_factor=0.005,color = (0,0,0))
    mlab.plot3d((center_point[0],Bottom_center[0]),(center_point[1],Bottom_center[1]),(center_point[2],Bottom_center[2]),tube_radius = 0.03, tube_sides = 20 ,color = (1,0,0), opacity = 0.5 )
    mlab.plot3d((center_point[0],Bottom_center[0]),(center_point[1],Bottom_center[1]),(center_point[2],Bottom_center[2]),tube_radius = 0.001, tube_sides = 20 ,color = (0,0,0), opacity = 1 )
def show_selected_grasps_with_color(m, ply_name_, title, obj_):
    #  m二维数组的倒数第二列表示fc，即摩擦系数   shape(1000,12)
    # m_good = m[m[:, -2] <= 1.8]   # 0.4
    # print('m_good',m_good)

    #  np.random.choice()   表示从m_good中随机抽取25个组成二维数组
    # m_good = m_good[np.random.choice(len(m_good), size=1, replace=True)]
    m_bad = m[m[:, -2] >= 1.8]
    m_bad = m[m[:,-2]<=0.1]
    m_bad = m_bad[np.random.choice(len(m_bad), size=5, replace=True)]
    ...
    if save_fig or show_fig:
        ...
        if save_fig:
            ...
            mlab.title(title+"_m_bad", size=0.3)
            mlab.show()
